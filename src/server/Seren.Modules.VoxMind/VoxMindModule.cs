...
/// <summary>
/// VoxMind module — wires the local Parakeet (STT) and F5-TTS (TTS) ONNX
/// engines into Seren via the standard <see cref="ISttProvider"/> /
/// <see cref="ITtsProvider"/> abstractions, registers the post-hoc language
/// detector, the metrics surface, the FluentValidation options adapter, and
/// (via <see cref="IHealthCheckProviderModule"/>) the per-bundle health probes.
/// </summary>
/// <remarks>
/// <para>
/// Configuration section: <c>Modules:voxmind</c>. When
/// <see cref="VoxMindOptions.Enabled"/> is <c>false</c>, the module is a no-op
/// and the host falls back to <c>AudioModule</c>'s NoOp / OpenAI providers.
/// </para>
/// <para>
/// The module uses <see cref="ServiceCollectionDescriptorExtensions.Replace"/>
/// to take precedence over the prior provider registrations from AudioModule
/// — order in <c>AddSerenModules(...)</c> is irrelevant for resolution.
/// </para>
/// </remarks>
public sealed class VoxMindModule : ISerenModule, IHealthCheckProviderModule
{
    /// <inheritdoc />
