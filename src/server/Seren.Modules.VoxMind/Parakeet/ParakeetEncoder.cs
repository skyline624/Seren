...
public sealed class ParakeetEncoder : IDisposable
{
    ...
    {
        var melTensor = new DenseTensor<float>(melFeatures, [1, NMels, (int)melFrames]);

        NamedOnnxValue lengthInput;
        if (_lengthIsInt64)
        {
            var lt = new DenseTensor<long>(new[] { melFrames }, [1]);
            lengthInput = NamedOnnxValue.CreateFromTensor(_lengthInputName, lt);
        }
        else
        {
            var lt = new DenseTensor<int>(new[] { (int)melFrames }, [1]);
            lengthInput = NamedOnnxValue.CreateFromTensor(_lengthInputName, lt);
        }
