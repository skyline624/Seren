...
/// <summary>
/// Stopword-overlap language detector — counts how many tokens of the input
/// match the stopword list of each candidate language and picks the highest
/// normalised match ratio.
/// </summary>
/// <remarks>
/// Why not an n-gram lib (NTextCat / cld3)? They require either a 5 MB
/// language profile on disk or a native binding. Here we only target the
/// 25 European languages Parakeet v3 supports on already-transcribed text
/// (well-formed, typical length &gt; 5 words) — the stopword approach is
/// sufficient, deterministic, AOT-compatible and zero-dep.
/// Ported from the upstream VoxMind <c>StopwordLanguageDetector</c>.
/// </remarks>
public sealed partial class StopwordLanguageDetector : ILanguageDetector
{
    /// <summary>Texts shorter than this many useful tokens are reported as undetermined.</summary>
    private const int MinUsefulTokens = 3;

    /// <summary>Minimum normalised score to consider a detection valid.</summary>
    ...
    public string DetectLanguage(string text, IEnumerable<string>? candidateCodes = null)
    {
        ...
        {
            _logger?.LogDebug(
                "Language detection: confidence too low (best={Code} score={Score:F2} on {N} tokens), returning 'und'.",
    ...
    /// <summary>
    /// Stopwords (top-30 frequent words) for the 25 European languages
    /// supported by Parakeet TDT v3. Sources: Wikipedia frequency lists +
    /// Tatoeba corpora. Codes are ISO 639-1.
    /// </summary>
    private static Dictionary<string, HashSet<string>> BuildStopwordTable()
    {
        return new Dictionary<string, HashSet<string>>(StringComparer.OrdinalIgnoreCase)
        {
            ["bg"] = Set("и", "в", "на", "е", "за", "се", "че", "не", "с", "по", "от", "като", "са", "до", "този", "така", "при", "който", "ще", "беше", "си", "но", "още", "ако", "те", "го", "много", "защото", "всичко"),
            ...
            ["et"] = Set("ja", "on", "ei", "et", "ta", "see", "ma", "kui", "olen", "siis", "ka", "aga", "või", "selle", "oma", "minu", "ole", "nii", "kõik", "ning", "veel", "üks", "ühe", "kus", "mis", "olid", "nad", "mind", "olnud"),
            ...
            ["de"] = Set("der", "die", "das", "und", "in", "zu", "den", "ist", "von", "nicht", "sich", "auch", "es", "auf", "für", "mit", "ein", "eine", "einen", "dem", "im", "an", "war", "wie", "aber", "nur", "noch", "wir", "ihr", "sie", "sind", "haben", "werden", "sein"),
            ...
            ["sl"] = Set("in", "v", "se", "je", "na", "da", "so", "za", "pa", "ne", "to", "s", "kot", "po", "ali", "če", "iz", "že", "samo", "še", "te", "biti", "ko", "tako", "kaj", "lahko", "ima", "pri"),
            ...
            ["ru"] = Set("и", "в", "не", "на", "я", "что", "он", "с", "по", "это", "она", "но", "за", "к", "из", "от", "у", "то", "его", "так", "же", "как", "бы", "только", "вы", "ты", "мы", "они", "был"),
